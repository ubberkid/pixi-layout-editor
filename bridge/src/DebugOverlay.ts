...
const DEFAULT_OPTIONS: Required<DebugOverlayOptions> = {
	fillColor: 0x0000ff,
	fillAlpha: 0.3,
...
export function addDebugOverlay(
	...
	(overlayContainer as Container & { measurable?: boolean }).measurable = false;
	...
	const resolveOverlaySize = () => {
		const layout = (container as Container & { layout?: { computedLayout?: { width: number; height: number } } }).layout;
	...
	(overlayContainer as Container & { cleanup: () => void }).cleanup = () => {
